...
const SinglePage = props => {
	...
	return (
		<div>
			...
			/>
			<div class={`block w-100 single-page ${props.location === 'mentors' ? 'my-1' : 'my-5' } mx-auto pb-5`}>
				{props.location !== 'mentors' &&
								<div class="block float-left w-50 pr-3">
									<Image name={item.image} location={props.location} class="mentors-image" />
									<div class="flex flex-ycenter pt-2 felx-start">
										<h5>{item.name} </h5><div> &nbsp; - {item.position}</div>
									</div>
								</div>
				}
				{props.location === 'mentors' && <h5 class="py-2">{item.name} </h5>}
				{item.text}
